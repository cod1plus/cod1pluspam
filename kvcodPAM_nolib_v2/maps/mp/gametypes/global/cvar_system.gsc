...
{
	cvar = maps\mp\gametypes\global\_global::toLower(cvar);
	...
	{
		logprint("Multiple registration of cvar " + cvar + ".");
	}
	#/
...
getValueByType(name, type, defaultValue)
{
	...
	else logprint("Cvar type " + type + " is unknown");
}
...
setClientCvarIfChanged(cvar, value)
{
	if (!isDefined(value)) // just safety
	{
		value = "";
		/#
		logprint("setClientCvarIfChanged() setting undefined value for cvar " + cvar);
		#/
	}
	/#
	if (!isPlayer(self)) // just safety
	{
		logprint("setClientCvarIfChanged() self is not a player");
	}
	#/
	...
	{
		/#
		logprint("setClientCvarIfChanged() value.size " + value.size + " > 1000");
		#/
		value = maps\mp\gametypes\global\_global::getsubstr(value, 0, 1000);
	}


	cvar = maps\mp\gametypes\global\_global::toLower(cvar);	// Convert to lower to match same cvars
	lastValue = self.pers["cvar_" + cvar]; // Get last value, may be undefined for first time

	// If value changed from last
	if (!isDefined(lastValue) || valueStr != lastValue)
	{
		//logprint("cvar=" + cvar + ", newValue=" + valueStr + "\n");
		self setClientCvar2(cvar, valueStr);
	}
	else
	{
		//logprint("cvar=" + cvar + ", lastValue=" + lastValue + ", newValue=" + valueStr + " didn't change\n");
	}
