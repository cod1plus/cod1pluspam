...
init()
{
	logprint("_overtime::init\n");
	
	maps\mp\gametypes\global\_global::addEventListener("onCvarChanged", ::onCvarChanged);

	maps\mp\gametypes\global\_global::registerCvar("scr_overtime", "BOOL", 0);
	...
	if(game["firstInit"])
	{
		...
		maps\mp\gametypes\global\_global::precacheString2("STRING_GOING_TO_OVERTIME", &"Going to Over-time");
	}
}
...
Do_Overtime()
{
	...
	{
		logprint("_overtime:: Invoked map_restart(true)\n");
        map_restart(true);
	}
}
