...
init()
{
	...
	maps\mp\gametypes\global\_global::registerCvar("scr_bash", "BOOL", 0);

	if(game["firstInit"])
	{
		maps\mp\gametypes\global\_global::precacheString2("STRING_BASH_MODE", &"Pistol bash");
	}
...
{
	logprint("_bash::onMenuResponse - " + menu + " " + response + "\n");
	/*
...
callbash()
{
	...
	if (action != "notallowed")
	{
		...
		{
			 players[i] maps\mp\gametypes\global\_global::setClientCvar2("ui_allow_bash", players[i] Validate_bash(false));
		}
	}
...
HUD_Bash()
{
	...
	{
		level.bash_hud_info = maps\mp\gametypes\global\_global::addHUD(320, 86, 1.1, (1,1,0), "center", "top");
		level.bash_hud_info setText(game["STRING_BASH_MODE"]);
	}
	else
	{
		level.bash_hud_info destroy();
